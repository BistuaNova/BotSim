    ...
    app.factory('State', function($window, $rootScope, ALL_SKILLS){
      ...
      function startBot(acc, scriptKey, script){ if(acc.banned || acc.sold) return false; if(acc.bot && acc.bot.running) return false; const sid=(crypto.randomUUID?crypto.randomUUID():String(Math.random()).slice(2)); acc.bot = { scriptKey, startedAt: Date.now(), xpPerMin: script.xpPerMin, skill: script.skill, costRunePerMin: script.costRunePerMin||2, running: true, sid }; acc.sessions.push({ sid, scriptKey, startedAt: Date.now(), endedAt: null, costPerMin: (script.costRunePerMin||2) }); acc.botUsage[scriptKey] = (acc.botUsage[scriptKey]||0)+1; STATE.totals.botsStarted += 1; save(); $rootScope.$broadcast('state:changed'); return true; }
    ...
    const Scripts = {
      woodcutting_basic:{ name:'Willow Chopper', skill:'Woodcutting', xpPerMin:30*60, risk:8, icon:'🌳', costRunePerMin:2, fav:false },
      fishing_basic:{ name:'Fly Fisher', skill:'Fishing', xpPerMin:28*60, risk:7, icon:'🐟', costRunePerMin:1.5, fav:false },
      mining_basic:{ name:'Iron Miner', skill:'Mining', xpPerMin:25*60, risk:9, icon:'⛏️', costRunePerMin:2.5, fav:false },
      cooking_basic:{ name:'Power Cooker', skill:'Cooking', xpPerMin:35*60, risk:6, icon:'🍳', costRunePerMin:1.2, fav:false }
    };
    ...
    app.component('botManager', {
      ...
      controller: function($scope, State){
        ...
        function computePairs(){ return Object.keys(Scripts).map(function(k){ return { key:k, s: Scripts[k] }; }).sort(function(a,b){ return ((b.s.fav?1:0)-(a.s.fav?1:0)) || a.s.name.localeCompare(b.s.name); }); }
        ...
        $scope.getSelectableAccounts = ()=> State.data.accounts.filter(a=> !a.banned && !a.sold && !(a.bot && a.bot.running));
        function isSelectable(a){ return a && !a.banned && !a.sold && !(a.bot && a.bot.running); }
        $scope.newAccount = ()=>{
        ...
        $scope.start = (key)=>{
          ...
          if(!isSelectable(acc)){ alert('Choose an account that is not banned/sold and not currently running a bot.'); $scope.selected[key]=null; return; }
          const need = s.costRunePerMin*1; if(State.data.economy.rune < need){ alert('Not enough RuneCoin to start this bot.'); return; }
          State.startBot(acc, key, s);
        };
        $scope.stop = (acc)=> State.stopBot(acc);
        $scope.$on('state:changed', ()=> { Object.keys($scope.selected).forEach(k=>{ const id=$scope.selected[k]; const acc=State.data.accounts.find(a=>a.id===id); if(!isSelectable(acc)) $scope.selected[k]=null; }); $scope.$evalAsync(); });
      }
    });
    ...
    app.component('accountManager', {
      ...
      controller: function($scope, State, ALL_SKILLS, SKILL_ICONS){
        ...
        $scope.sortedAccounts = ()=>{
          ...
          const get = (a)=> key==='name'? a.name.toLowerCase() : key==='totalLevel'? State.totalLevel(a) : key==='h24'? State.hoursBotted24h(a) : key==='hall'? State.hoursBottedTotal(a) : getStatus(a);
          return arr.sort((a,b)=> ((b.fav?1:0)-(a.fav?1:0)) || ((get(a)>get(b)?1:-1)*dir));
        };
        $scope.toggleFav = (acc)=>{ acc.fav = !acc.fav; State.save(); };
    ...
    app.run(function($rootScope, $interval, State){
      ...
      function tick(){ State.data.accounts.forEach(acc=>{ if(acc.banned) return; if(acc.bot && acc.bot.running){ const perSec = (acc.bot.xpPerMin/60); acc.skills[acc.bot.skill] = Math.round((acc.skills[acc.bot.skill]||0) + perSec); } computeRisk(acc); const danger=Math.max(0, acc.risk-60); if(danger>0 && Math.random() < (danger/700)){ acc.banned=true; $('#ban-msg').innerHTML = `Account <b>${acc.name}</b> has been banned.`; $('#ban-overlay').style.display='flex'; const sess=[...acc.sessions].reverse().find(s=>!s.endedAt); if(sess){ sess.endedAt=Date.now(); acc.bot && (acc.bot.running=false); State.upsertSessionTxn(acc, sess, true); } } }); State.save(); $rootScope.$broadcast('state:changed'); }
      $interval(tick, 1000);
    ...
    const Apps = {
      ...
      'Browser': { title:'Web Browser', icon:'🌐', size:{ w:740, h:480 }, mountTag:`<div><div class="browser-top"><input value="home://" id="addr" spellcheck="false"><button id="go">Go</button></div><div class="tabbar" id="tabbar"></div><div class="browser-view" id="view"></div></div>` },
      'Wallet': { title:'Crypto Wallet', icon:'👛', size:{ w:720, h:520 }, mountTag:'<wallet-app></wallet-app>' },
      'Notes': { title:'Notes', icon:'🗒️', size:{ w:520, h:360 }, mountTag:`<textarea style="width:100%;height:100%;background:var(--bg-soft);color:var(--text);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:10px;resize:none;"></textarea>` }
    };
    window.Apps = Apps;

    // Fake browser init
    function initBrowser(content){ const allowed={ 'home://':`<h2>Start Page</h2><p>Welcome to <b>GielinorNet</b>. Try visiting:</p><ul><li><code>wiki://woodcutting</code></li><li><code>wiki://fishing</code></li><li><code>forum://scripts</li></ul><p class='small'>This is a fake, sandboxed browser for the simulator.</p>`, 'wiki://woodcutting':`<h2>Wiki — Woodcutting</h2><p>Willow trees give fast XP but low profit.</p>`, 'wiki://fishing':`<h2>Wiki — Fishing</h2><p>Fly fishing is consistent XP.</p>`, 'forum://scripts':`<h2>Bot Scripts Forum</h2><p>Top Free Scripts:</p><ol><li>Willow Chopper</li><li>Fly Fisher</li><li>Iron Miner</li></ol>` }; const top=content.querySelector('#addr'); const go=content.querySelector('#go'); const tabbar=content.querySelector('#tabbar'); const view=content.querySelector('#view'); const tabs=[]; function openTab(url){ const t={url, el:el('div',{className:'tab'})}; t.el.append(el('span',{textContent:'🗂️'}), el('span',{textContent:url}), el('span',{className:'x',textContent:'✕'})); tabbar.append(t.el); t.el.addEventListener('click',(e)=>{ if(e.target.classList.contains('x')) closeTab(t); else setActive(t); }); tabs.push(t); setActive(t);} function setActive(t){ tabs.forEach(tt=>tt.el.classList.toggle('active', tt===t)); top.value=t.url; renderUrl(t.url);} function closeTab(t){ const i=tabs.indexOf(t); if(i>=0){ tabs.splice(i,1); t.el.remove(); tabs.length? setActive(tabs[Math.max(0,i-1)]): openTab('home://'); }} function renderUrl(u){ view.innerHTML = allowed[u] || `<h2>Blocked</h2><p>Navigation to <code>${u}</code> is blocked by your ISP.</p>`; } go.addEventListener('click',()=>openTab(top.value.trim())); top.addEventListener('keydown',(e)=>{ if(e.key==='Enter') openTab(top.value.trim()); }); openTab('home://'); }

    // Desktop icons & start menu
    const ICONS = [ { title:'Bot Manager', icon:'🤖' }, { title:'Account Manager', icon:'📈' }, { title:'Browser', icon:'🌐' }, { title:'Wallet', icon:'👛' }, { title:'Notes', icon:'🗒️' }, { title:'Trash', icon:'🗑️', action:()=>toast('Trash is empty.') } ];
    function drawDesktop(){
      ...
      for(const ic of ICONS){ const div=el('div',{className:'desktop-icon',tabIndex:0}); div.append(el('div',{className:'icon-emoji',textContent:ic.icon}), el('div',{className:'icon-label',textContent:ic.title})); div.addEventListener('dblclick',()=>{ if(ic.action) ic.action(); else { const w=Desktop.spawn(ic.title); if(ic.title==='Browser') initBrowser(w.content); } }); d.append(div);} }
    ...
    $('#start-menu').addEventListener('click',(e)=>{ const row=e.target.closest('.srow'); if(!row) return; const cmd=row.dataset.cmd||''; const State = angular.element(document.body).injector().get('State'); if(cmd.startsWith('open:')){ const id=cmd.split(':')[1]; const w=Desktop.spawn(id); if(id==='Browser') initBrowser(w.content); } else if(cmd==='reset'){ if(confirm('Reset all saves?')){ localStorage.removeItem('rs-bot-sim'); location.reload(); } } else if(cmd==='wallpaper'){ State.data.wallpaper=(State.data.wallpaper+1)%3; State.save(); drawDesktop(); } });

    document.addEventListener('keydown',(e)=>{ if(e.shiftKey){ const k=e.key.toLowerCase(); if(k==='b') Desktop.spawn('Bot Manager'); if(k==='s') Desktop.spawn('Account Manager'); if(k==='w') { const w=Desktop.spawn('Browser'); initBrowser(w.content); } if(k==='r'){ Desktop.spawn('Wallet'); } }});
